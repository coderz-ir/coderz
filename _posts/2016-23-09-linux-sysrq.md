...
* `R`: un`R`aw      (پس گرفتن کنترل کیبورد)
* `E`: t`E`rminate (ارسال SIGTERM به تمام پروسه ها)
* `I`: k`I`ll      (ارسال SIGKILL به تمام پروسه ها)
* `S`:  `S`ync     (فلاش دیتای ذخیره نشده در دیسک)
* `U`:  `U`nmount
* `B`:re`B`oot.
